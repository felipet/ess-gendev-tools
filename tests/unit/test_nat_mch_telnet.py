...
class TestNATMCHTelnet:
    def setup(self):
        self.valid_mch = NATMCHTelnet(config["Metadata"]["valid_ip_address"])

    def test_timeout(self):
        """Test the access timeout to a device that is down"""
        with pytest.raises(ConnTimeout):
            self.invalid_mch = NATMCHTelnet(config["Metadata"]["invalid_ip_address"])

    def test_device_info(self):
        """Test the device_info feature when using Telnet"""
        device_info = self.valid_mch.device_info()
        expected = {}
        # Subset the entire configuration for the comparison
        expected["Board"] = config["Board"]
        expected["Network"] = config["Network"]
        assert expected == device_info

    def test_update_fw(self):
        """Test the update_fw feature when using Telnet"""
        response = self.valid_mch.update_fw(
            config["Metadata"]["update_fw"]["invalid_fw"]
        )
